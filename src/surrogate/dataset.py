...
class TransitionDataset:
    ...
    def normalize_sa(self, states: np.ndarray, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        s_norm = np.nan_to_num(s_norm, nan=0.0, posinf=1e6, neginf=-1e6)
        a_norm = np.nan_to_num(a_norm, nan=0.0, posinf=1e6, neginf=-1e6)
...
def build_dataset(transitions: List[Tuple[np.ndarray, np.ndarray, np.ndarray]]) -> TransitionDataset:
    ...
    deltas = np.nan_to_num(deltas, nan=0.0, posinf=1e6, neginf=-1e6)
    states = np.clip(states, -1e6, 1e6)
    actions = np.clip(actions, -1e6, 1e6)
    deltas = np.clip(deltas, -1e6, 1e6)

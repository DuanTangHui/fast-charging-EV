...
class LiionpackSPMEPackEnv(BasePackEnv):
    """Pack environment wrapper with liionpack/pybamm backend or toy fallback."""
    ...
    def __init__(
        ...
        self._use_liionpack = bool(_HAS_LIIONPACK and backend == "liionpack")
        self._cell_sims = None
    ...
    def reset(self, *, seed: int | None = None, options: Dict | None = None) -> Tuple[np.ndarray, Dict]:
        ...
        if self._use_liionpack:
            try:
                return self._reset_liionpack()
            except Exception:
                self._use_liionpack = False
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if self._use_liionpack:
            try:
                return self._step_liionpack(action)
            except Exception:
                self._use_liionpack = False
    ...
    def _reset_liionpack(self) -> Tuple[np.ndarray, Dict]:
        if not _HAS_LIIONPACK:
            raise RuntimeError("liionpack/pybamm not available")
        soc = self._rng.uniform(0.2, 0.3, size=self.n_cells)
        temperature = 298 + self._rng.normal(0, 0.5, size=self.n_cells)
        soh = np.ones(self.n_cells) * self._aging.capacity_scale
        self._cell_sims = []
        ...
        for i in range(self.n_cells):
            ...
            params.update(
                {
                    "Nominal cell capacity [A.h]": float(
                        params["Nominal cell capacity [A.h]"] * self._aging.capacity_scale
                    ),
                    "Current function [A]": pybamm.InputParameter("Current function [A]"),
                }
            )
            sim = pybamm.Simulation(model, parameter_values=params, solver=solver)
            self._cell_sims.append(sim)
        voltage = 3.5 + 0.1 * soc
        self._state = PackState(soc, voltage, temperature, soh, 0.0, 0.0, 0)
        obs = build_observation(soc, voltage, temperature, soh, 0.0).as_array()
        info = {
    ...
    def _step_liionpack(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        if self._state is None or self._cell_sims is None:
            raise RuntimeError("Environment must be reset before step.")
        current = float(np.clip(action[0], self.action_space.low[0], self.action_space.high[0]))
        soc = []
        voltage = []
        temperature = []
        for sim in self._cell_sims:
            sol = sim.solve(
                t_eval=[0, self.dt],
                inputs={"Current function [A]": current},
            )
            soc.append(float(sol["State of Charge"].data[-1]))
            voltage.append(float(sol["Terminal voltage [V]"].data[-1]))
            temperature.append(298.0 + 0.05 * current)
        ...
        obs = build_observation(soc_arr, voltage_arr, temperature_arr, soh, current).as_array()
        v_max = float(np.max(voltage_arr))
        t_max = float(np.max(temperature_arr))
        violation = v_max > self.v_max or t_max > self.t_max
        terminated = self._state.step >= self.max_steps or (self.terminate_on_violation and violation)
        truncated = False
        info = {
            "t": self._state.t,
            "I": current,
            "SOC_pack": float(np.mean(soc_arr)),
            "V_cell_max": v_max,
            "V_cell_min": float(np.min(voltage_arr)),
            "T_cell_max": t_max,
            "T_cell_min": float(np.min(temperature_arr)),
            "dV": float(np.max(voltage_arr) - np.min(voltage_arr)),
            "dT": float(np.max(temperature_arr) - np.min(temperature_arr)),
            "terminated_reason": "violation" if violation else "time",
            "violation": violation,
        }
        return obs, 0.0, terminated, truncated, info
...
def build_pack_env(config: Dict) -> LiionpackSPMEPackEnv:
    ...
    env = LiionpackSPMEPackEnv(
        ...
        backend=str(config.get("backend", "toy")),
    )
    return env

...
from ..rewards.paper_reward import PaperRewardConfig, reward_from_info, compute_paper_reward, compute_minimal_reward
...
def rollout_surrogate(
    ...
    # curr_info["i_abs"] = float(abs(curr_state[IDX_IPREV]))

    infos.append(curr_info)
  
    # 软约束参数
    v_soft_thr = v_max - 0.05 # 比硬约束低一点，比如 4.17
    t_soft_thr = t_max - 5.0  # 比硬约束低一点，比如 318.5K

    # 定义Guard：
    def guard_action(action: float, info: dict) -> float:
        if info.get("viol_hard", False):
            return float(np.clip(action + 5.0, -20, 0))
        if info.get("V_cell_max", -1e9) >= v_soft_thr:
            return float(np.clip(action + 2.0, -20, 0))
        if info.get("T_cell_max", -1e9) >= t_soft_thr:
            return float(np.clip(action + 2.0, -20, 0))
        return action
    ...
    for k in range(horizon):
        ...
        # 修建动作
        # a_val = guard_action(a_val, infos[-1])
        a_val = float(np.clip(a_val, -20, 0))

        # 2. 预测 (Mean, Std)
        a_arr = np.array([a_val], dtype=np.float32)
        delta_mean, delta_std = surrogate(curr_state, a_arr)
      
        # 3. 状态更新 只更新前6维；Iprev=action）
        next_state = curr_state.copy()
        # 【核心修改】：强制 SOC (索引0) 的增量非负
        ...
        # 5. 计算奖励
        # r_phys, terms = compute_paper_reward(
        ...
        #     config=reward_cfg
        # )
        r_phys = compute_minimal_reward(
            soc_prev=curr_state[IDX_SOC],
            soc_next=next_state[IDX_SOC],
            v_max=next_state[IDX_VMAX],
            t_max=next_state[IDX_TMAX],
            v_limit=v_max,
            t_limit=t_max
        )
        # 【关键修改 2】risky 只加惩罚，不终止
        risk_penalty = -10.0 if is_risky else 0.0
        hard_penalty = -80.0 if viol_hard else 0.0
        
        step_reward = r_phys + risk_penalty + hard_penalty
        total_reward += step_reward

        # 6) violation 与终止条件（尽量贴近真实环境口径）
        soc_done = float(next_state[IDX_SOC]) >= 0.98
        ...
        # step_info.update(BASE_TERMS)
        # step_info.update(terms)
